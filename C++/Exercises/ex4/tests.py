...
USE_VALGRIND_EXTRA = False # Whether to run under Valgrind early exits (bad _matrix operations, etc.) as well
...
DUMP_FILES_INFO = f"The expected _matrix has been dumped textually into the file \"{EXPECTED_RESULTS_FILE}\", and the actual _matrix into \"{ACTUAL_RESULTS_FILE}\". " + \
...
INP_FILE = "inp"
INP_FILE_INFO = f"The _matrix used for the test has been dumped textually into the file \"{INP_FILE}\". It can be viewed as text and loaded in Python with np.loadtxt().\n"
INP_FILE_SHORT_INFO = f"The _matrix used for the test has been dumped textually into the file \"{INP_FILE}\".\n"
...
class MatrixTests:
    ...
    def matctor0():
        ...
        retcode = proc.wait()
        if retcode != 0:
            sys.stderr.write(red(f"Default-construction of a _matrix caused your code to exit with code {retcode}.\n"))
            ...
            sys.stderr.write(red(f"Default-construction of a _matrix yielded a _matrix with dimensions {actual_mat.shape}.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
        ...
        if result_err_text:
            sys.stderr.write(red(f"Default-construction of a _matrix caused your code to print to standard error.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
    ...
    def matctor1():
        ...
        retcode = proc.wait()
        if retcode != 0:
            sys.stderr.write(red(f"Copy-construction of a _matrix with dimensions {mat.shape} caused your code to exit with code {retcode}.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            if USE_VALGRIND:
        ...
        if not np.allclose(mat, actual_mat, rtol = MatrixTests.MATCTOR_REL_DEV, atol = MatrixTests.MATCTOR_ABS_DEV):
            ...
            sys.stderr.write(red(f"Copy-construction of a _matrix with dimensions {mat.shape} yielded a highly deviant results (abs avg dev = {avg_dev}).\n"))
        ...
        if result_err_text:
            sys.stderr.write(red(f"Copy-construction of a _matrix with dimensions {mat.shape} caused your code to print to standard error.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
    ...
    def valid_matctor2():
        ...
        if retcode != 0:
            sys.stderr.write(red(f"Construction of a _matrix with valid dimensions {dims} caused your code to exit with code {retcode}.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            if USE_VALGRIND:
        ...
        if actual_mat.shape != dims:
            sys.stderr.write(red(f"Construction of a _matrix with valid dimensions {dims} yielded a _matrix with dimensions {actual_mat.shape}.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
        ...
        if result_err_text:
            sys.stderr.write(red(f"Construction of a _matrix with valid dimensions {dims} caused your code to print to standard error.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
    ...
    def premature_eof():
        ...
        if retcode != 1:
            sys.stderr.write(red(f"Premature EOF when loading {bad_shape[0] * bad_shape[1]} elements "\
                                 f"into a _matrix with dimensions {mat.shape} (streaming less data than reported) didn't cause an exit with code 1 (but rather {retcode}).\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            if USE_VALGRIND:
        ...
        if result_out_text:
            sys.stderr.write(red(f"Upon a premature EOF when loading {bad_shape[0] * bad_shape[1]} elements "\
                                 f"into a _matrix with dimensions {mat.shape} (streaming less data than reported), your code printed to standard output.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
        ...
        if not (result_err_text.startswith("Error:") and result_err_text.endswith('\n')):
            sys.stderr.write(red(f"Upon a premature EOF when loading {bad_shape[0] * bad_shape[1]} elements "\
                                 f"into a _matrix with dimensions {mat.shape} (streaming less data than reported), your code should print to standard error " \
    ...
    def missing_eof():
        ...
        if retcode != 1:
            sys.stderr.write(red(f"Missing EOF when loading {bad_shape[0] * bad_shape[1]} elements "\
                                 f"into a _matrix with dimensions {mat.shape} (streaming more data than reported) didn't cause an exit with code 1 (but rather {retcode}).\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            if USE_VALGRIND:
        ...
        if result_out_text:
            sys.stderr.write(red(f"Upon withholding EOF when loading {bad_shape[0] * bad_shape[1]} elements "\
                                 f"into a _matrix with dimensions {mat.shape} (streaming more data than reported), your code printed to standard output.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
        ...
        if not (result_err_text.startswith("Error:") and result_err_text.endswith('\n')):
            sys.stderr.write(red(f"Upon withholding EOF when loading {bad_shape[0] * bad_shape[1]} elements "\
                                 f"into a _matrix with dimensions {mat.shape} (streaming more data than reported), your code should print to standard error " \
                                  "a message starting with \"Error:\" and ending with a newline.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
    ...
    def valid_scalar_mat_mul(left = True):
        ...
        retcode = proc.wait()
        if retcode != 0:
            sys.stderr.write(red(f"Multiplication of _matrix with dimensions {mat.shape} with scalar caused your code to exit with code {retcode}.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            if USE_VALGRIND:
        ...
        if result_err_text:
            sys.stderr.write(red(f"Multiplication of _matrix with dimensions {mat.shape} with scalar caused your code to print to standard error.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
        ...
        if not np.allclose(expected_mat, actual_mat, rtol = MatrixTests.SCALAR_MAT_MUL_REL_DEV, atol = MatrixTests.SCALAR_MAT_MUL_ABS_DEV):
            avg_dev = np.abs(np.average(expected_mat - actual_mat))
            if left:
                sys.stderr.write(red(f"Multiplication of _matrix with dimensions {mat.shape} with scalar yielded a highly deviant results (abs avg dev = {avg_dev}).\n"))
            else:
                sys.stderr.write(red(f"Multiplication of scalar with _matrix with dimensions {mat.shape} yielded a highly deviant results (abs avg dev = {avg_dev}).\n"))
    ...
    def valid_getitem(flat = False):
        ...
        retcode = proc.wait()
        if retcode != 0:
            sys.stderr.write(red(f"""{"Flat-access" if flat else "Access"} to a valid item in _matrix with dimensions {mat.shape} caused your code to exit with code {retcode}.\n"""))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            if USE_VALGRIND:
        ...
        if result_err_text:
            sys.stderr.write(red(f"""{"Flat-access" if flat else "Access"} to a valid item in _matrix with dimensions {mat.shape} caused your code to print to standard error.\n"""))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
        ...
        for i, equal_elem in enumerate(equal_arr):
            if not equal_elem:
                sys.stderr.write(red(f"""{"Flat-access" if flat else "Access"} to valid item in _matrix with dimensions {mat.shape} """ \
    ...
    def valid_setitem(flat = False):
        ...
        retcode = proc.wait()
        if retcode != 0:
            sys.stderr.write(red(f"""Mutating an item in a _matrix with dimensions {mat.shape}{" via flat-access" if flat else ""} caused your code to exit with code {retcode}.\n"""))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            if USE_VALGRIND:
        ...
        if result_err_text:
            sys.stderr.write(red(f"""Mutating an item in a _matrix with dimensions {mat.shape}{" via flat-access" if flat else ""} caused your code to print to standard error.\n"""))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
        ...
        for coord, equal_elem in np.ndenumerate(equal_mat):
            ...
            if not equal_elem:
                sys.stderr.write(red(f"""Mutating an item in a _matrix with dimensions {mat.shape}{" via flat-access" if flat else ""} yielded a highly deviant entry at {coord} in the _matrix """ \
    ...
    def invalid_matctor2():
        ...
        if retcode != 1:
            sys.stderr.write(red(f"Construction of _matrix with invalid dimensions {dims} didn't cause an exit with code 1 (but rather {retcode}).\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            if USE_VALGRIND:
        ...
        if result_out_text:
            sys.stderr.write(red(f"In construction of _matrix with invalid dimensions {dims}, your code printed to standard output.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            TEST_FAILED(test_name)
        ...
        if not (result_err_text.startswith("Error:") and result_err_text.endswith('\n')):
            sys.stderr.write(red(f"In construction of _matrix with invalid dimensions {dims}, your code should print to standard error " \
                                  "a message starting with \"Error:\" and ending with a newline.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
    ...
    def invalid_matmul():
        ...
        while mat1.shape[1] == mat2.shape[0]: # Guarantee invalid _matrix multiplication
            mat2 = MatrixTests.randmat(MatrixTests.MATMUL_DIM_BOUNDS, MatrixTests.MATMUL_ENTRY_BOUNDS)
    ...
    def invalid_matadd(inplace = False):
        ...
        while mat1.shape == mat2.shape: # Guarantee invalid _matrix addition
            mat2 = MatrixTests.randmat(MatrixTests.MATMUL_DIM_BOUNDS, MatrixTests.MATMUL_ENTRY_BOUNDS)
    ...
    def invalid_getitem(flat = False):
        ...
        if retcode != 1:
            sys.stderr.write(red(f"""{"Flat-access" if flat else "Access"} to an out-of-bounds item{(" [" + str(index) + "] ==") if flat else ""} """
                                 f"""{(index // mat.shape[1], index % mat.shape[1])} in _matrix with dimensions {mat.shape} caused your code to exit with code {retcode}.\n"""))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
            if USE_VALGRIND:
        ...
        if not (result_err_text.startswith("Error:") and result_err_text.endswith('\n')):
            sys.stderr.write(red(f"""In {"flat-access" if flat else "access"} to an out-of-bounds item{(" [" + str(index) + "] ==") if flat else ""} """
                                 f"""{(index // mat.shape[1], index % mat.shape[1])} in _matrix with dimensions {mat.shape}, your code should print to standard error """ \
                                  "a message starting with \"Error:\" and ending with a newline.\n"))
            with open(STDERR_DUMP_FILE, 'w') as stderr_dump_wfd:
                stderr_dump_wfd.write(result_err_text)
            sys.stderr.write(STDERR_FILE_INFO)
    ...
    def __init__(self, tests_type):
        ...
        if tests_type == "valid_construction":
            ...
            sys.stderr.write("[Premature EOF when loading _matrix: operator>> between istream and Matrix]\n")
            ...
            sys.stderr.write("[Missing EOF at end when loading _matrix: operator>> between istream and Matrix]\n")
            with tqdm.tqdm(total = MatrixTests.MATIO_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.MATIO_ITERS):
            ...
            sys.stderr.write("[Matrix-scalar multiplication: operator* between _matrix and scalar]\n")
            ...
            sys.stderr.write("[Scalar-_matrix multiplication: operator* between scalar and _matrix]\n")
            with tqdm.tqdm(total = MatrixTests.SCALAR_MAT_MUL_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.SCALAR_MAT_MUL_ITERS):
            ...
            sys.stderr.write("[In-place _matrix addition: operator+= between matrices]\n")
            with tqdm.tqdm(total = MatrixTests.MATADD_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.MATADD_ITERS):
            ...
            sys.stderr.write("[Matrix flat-access: operator[] on _matrix]\n")
            ...
            sys.stderr.write("[Matrix access: operator() on _matrix]\n")
            with tqdm.tqdm(total = MatrixTests.GETITEM_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.GETITEM_ITERS):
            ...
            sys.stderr.write("[Matrix flat-access mutation: setting item via operator[] on _matrix]\n")
            ...
            sys.stderr.write("[Matrix mutation: setting item via operator() on _matrix]\n")
            with tqdm.tqdm(total = MatrixTests.SETITEM_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.SETITEM_ITERS):
            ...
            sys.stdout.write(green("Valid _matrix arithmetic tests passed succesfully.\n"))

        elif tests_type == "invalid_arithmetic":
            sys.stdout.write(title("INVALID MATRIX ARITHMETIC TESTS"))

            sys.stderr.write("[Invalid _matrix multiplication: operator* between incompatible matrices]\n")
            with tqdm.tqdm(total = MatrixTests.MATMUL_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.MATMUL_ITERS):
            ...
            sys.stderr.write("[Invalid _matrix addition: operator+ between incompatible matrices]\n")
            with tqdm.tqdm(total = MatrixTests.MATADD_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.MATADD_ITERS):
            ...
            sys.stderr.write("[Invalid in-place _matrix addition: operator+= between incompatible matrices]\n")
            with tqdm.tqdm(total = MatrixTests.MATADD_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.MATADD_ITERS):
            ...
            sys.stderr.write("[Invalid _matrix flat-access: out-of-bounds operator[] on matrices]\n")
            with tqdm.tqdm(total = MatrixTests.GETITEM_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.GETITEM_ITERS):
            ...
            sys.stderr.write("[Invalid _matrix access: out-of-bounds operator() on matrices]\n")
            with tqdm.tqdm(total = MatrixTests.GETITEM_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.GETITEM_ITERS):
            ...
            sys.stderr.write("[Invalid _matrix flat-access mutation: out-of-bounds operator[] on matrices]\n")
            with tqdm.tqdm(total = MatrixTests.SETITEM_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.SETITEM_ITERS):
            ...
            sys.stderr.write("[Invalid _matrix mutation: out-of-bounds operator() on matrices]\n")
            with tqdm.tqdm(total = MatrixTests.SETITEM_ITERS, file = sys.stderr, leave = False) as pbar:
                for _ in range(MatrixTests.SETITEM_ITERS):
            ...
            sys.stdout.write(green("Invalid _matrix arithmetic tests passed succesfully.\n"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--valid-construction", help = "Run valid _matrix construction tests.", action = "store_true")
    parser.add_argument("--invalid-io", help = "Run invalid _matrix IO tests.", action = "store_true")
    parser.add_argument("--invalid-construction", help = "Run invalid _matrix construction tests.", action = "store_true")
    parser.add_argument("--valid-arithmetic", help = "Run valid _matrix arithmetic tests.", action = "store_true")
    parser.add_argument("--invalid-arithmetic", help = "Run invalid _matrix arithmetic tests.", action = "store_true")
    ...
    for i, arg in enumerate(sys.argv[1:]):
        if (arg == "--valid-construction" or args.all) and "--valid-construction" not in performed:
            # Test valid _matrix construction
        ...
        if (arg == "--invalid-construction" or args.all) and "--invalid-construction" not in performed:
            # Test invalid _matrix construction
        ...
        if (arg == "--invalid-arithmetic" or args.all) and "--invalid-arithmetic" not in performed:
            # Test invalid _matrix arithmetic
